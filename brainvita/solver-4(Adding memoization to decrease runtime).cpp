...
inline ll get(int r, int c){ return state & (1LL<<(MAX*r + c)); }

inline void st(int r, int c){ state |= (1LL<<(MAX*r + c)); }

inline void unst(int r, int c){ state &= ~(1LL<<(MAX*r + c)); }

inline int valid(int r, int c){ return (r>=0 && r<MAX && c>=0 && c<MAX && (abs(r-3)<2 || abs(c-3)<2)); }
